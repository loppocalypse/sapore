...
const COLOR_SCHEME = {
  light: {
    particle: {
      color: "rgba(0, 0, 0, 0.07)", // Açık modda koyu parçacıklar
    },
    background: "rgba(255, 255, 255, 0.12)", // Açık modda beyaz arka plan
  },
  dark: {
    particle: {
      color: "rgba(255, 255, 255, 0.07)", // Karanlık modda açık parçacıklar
    },
    background: "rgba(0, 0, 0, 0.12)", // Karanlık modda siyah arka plan
  },
} as const;
...
useEffect(() => {
  ...
  const animate = () => {
    ...
    const scheme = isDark ? COLOR_SCHEME.dark : COLOR_SCHEME.light; // Ters mantığı kaldırdık

    ctx.fillStyle = scheme.background; // Doğru arka plan rengi
    ...
    for (const particle of particles) {
      ...
      ctx.fillStyle = scheme.particle.color.replace("0.07", opacity.toString()); // Parçacık rengi
      ctx.beginPath();
  ...
  return (
    <div className={cn("relative w-full h-screen overflow-hidden", " bg-black dark:bg-black", className)}>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full h-full">
        ...
        >
          <h1 className="text-6xl md:text-8xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-white to-white/70 dark:from-black dark:to-black/70 drop-shadow-sm">
            {title}
          </h1>
          <Link
            target="_blank"
            ...
            className="text-xl md:text-2xl font-medium bg-clip-text text-transparent bg-gradient-to-b from-white/90 to-white/50 dark:from-black/90 dark:to-black/50 flex items-center justify-center"
          >
            {subtitle}
          </Link>
        </motion.div>
